<?php
	...
	class Index extends App_Controller
	{
		...
		function add()
		{
			$this->edit($this->db->getOne("SELECT MAX(id_i18n) FROM ".PRE."musca_i18n_admin")+1);
		}
		...
		function block($section=false, $lang_start=false, $lang_end=false)
		{
			...
			if(!empty($_POST['send']))
			{
				...
				foreach($tags as $tag => $value)
				{
				    $update = array();
				    $update['value'] = $value;
				    $this->db->update(PRE.'musca_i18n_admin',$update,"section='$section' AND lang='$lang' AND tag='$tag'");
				}
				$this->smarty->assign('saved', true);
			}
		    ...
		    $res = $this->db->getAll("SELECT * FROM ".PRE."musca_i18n_admin WHERE section='$section' AND lang in ('$lang_start', '$lang_end') ORDER BY id_i18n");
		    $return = array();
		    foreach($res as $k => $v) 
		    {
		...
		function del()
		{
			...
			$del = $this->db->getAssoc("SELECT tag, section FROM ".PRE."musca_i18n_admin WHERE id_i18n IN (".rtrim($_POST['ids'],",").")");
