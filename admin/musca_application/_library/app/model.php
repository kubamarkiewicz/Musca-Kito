<?php
	...
	*/
		...
		protected function getI18n($result, $table, $id)
		{
		...
		protected function saveI18n($i18n, $table, $id)
		{
		...
		protected function saveFile($id, $field, $parameters)
		{
			...
			// i18n
			if (is_array($_FILES['i18n']['name'][$field])) return $this->saveFileI18n($id, $field, $parameters);
		...
		protected function deleteFile($id, $field, $parameters)
		{
		...
		/* upload file and save in db with i18n */
		protected function saveFileI18n($id, $field, $parameters)
		{
			...
			{
				$filename = Musca_Utils_Upload::uploadFile('i18n', $filePath, $field, $iso);
				if ($filename) $this->db->query("INSERT INTO " . $this->table. "_i18n (id,lang,field,value) VALUES ($id,'$iso','$field','$filename') ON DUPLICATE KEY UPDATE value='$filename'");
			}
		}
		...
		protected function saveImage($id, $field, $parameters)
		{
			// print_r($_FILES); exit;
			if (!$parameters) return;
		...
		protected function deleteImage($id, $field, $parameters)
		{
			if (!$_POST[$field.'_del']) return;
		...
		protected function saveGallery($id, $config)
		{
			if (!$config) return;
