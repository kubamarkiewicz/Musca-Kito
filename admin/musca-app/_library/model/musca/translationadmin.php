	...
	class Model_Musca_TranslationAdmin
	{
		...
		function save($data, $id=false)
		{
		    ...
		    if ($data['langs']) foreach($data['langs'] as $lang => $val)
		    {
		        ...
		        $id_exist = $this->db->getOne("SELECT id_i18n FROM musca_i18n_admin WHERE section='".$data['section']."' AND tag='".$data['tag']."' AND lang='".$lang."'");
		        if (!empty($id_exist)) $this->db->update('musca_i18n_admin', $insert, "id_i18n=$id_exist");
				else $this->db->insert('musca_i18n_admin', $insert);
			}
		}
		
		function last() { return $this->db->getOne("SELECT MAX(id_i18n_lang) FROM musca_i18n_lang_admin"); }

		function get($id=false)
		{
			if (!$id) die('No existe ID');
			$elem = $this->db->getRow("SELECT * FROM musca_i18n_admin WHERE id_i18n=$id");
			$elem['langs'] = $this->db->getAssoc("SELECT lang, value FROM musca_i18n_admin WHERE section='".$elem['section']."' AND tag='".$elem['tag']."'");
		...
		{
		    return $this->db->getAssoc("SELECT DISTINCT section, section FROM musca_i18n_admin ORDER BY section");
		}
		...
		{
			$sql = "SELECT * FROM musca_i18n_admin WHERE tag='".mysql_real_escape_string($tag)."' LIMIT 1";
