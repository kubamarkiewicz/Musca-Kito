...
<form id="form_send" method="post" enctype="multipart/form-data">
	...
	<fieldset>
   		<input type="hidden" name="name" value="{$elem.name[$langs.0]}" id="duplicate_out" />
				...
				<fieldset>
					<table width="95%">
						<tr>
							...
							<td><input type="text" name="i18n[name][{$iso}]" value="{$elem.name.$iso}" id="duplicate_in_{$iso}" style="width: 100%" /></td>
						</tr>
						<tr>
			...
			<tr>
			    ...
			    <td>
					...
					<p class="mensaje">
						...
						{t section="$modul"}Image will be resized to{/t} <strong>{$imageConfig.width}x{$model->fields.image.height}</strong> px. {t section="$modul"}Maximum file size is{/t} <strong>{$MAX_FILE_SIZE_HUMAN}</strong>.
					</p>
				</td>
			</tr>
            ...
            <table>
                <tr><td>
			        {foreach $elem.gallery as $item}
			            <div class="image_box">
                            <div class="image">
                                <a href="{$musca_url}{$content_dir}{$galleryConfig.dir}/{$elem.id}/{$item.image}" target="_blank"><img src="{$musca_url}{$content_dir}{$galleryConfig.dir}/{$elem.id}/{$item.image}" /></a>
							</div>
							<div class="text_box">
...
{/*}
			...
			<p class="mensaje">
				{t section="gallery"}Images will be resized to{/t} <strong>{$galleryConfig.width}x{$galleryConfig.height}</strong> px. 
