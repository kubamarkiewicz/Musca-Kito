<?php
	...
	class Index extends App_Controller
	{
		...
		protected $flex_table = 'musca_i18n';
		...
		{
			return explode(',', TRANSLATIONS_LANGS);
		}
		...
		{	
			...
			$this->smarty->assign('langs', $this->getLangs());
			$admin = new Model_Musca_Translation($this->db);
		...
		function add()
		{
			$this->edit($this->db->getOne("SELECT MAX(id_i18n) FROM musca_i18n")+1);
		}
		...
		function block($section=false, $lang_start=false, $lang_end=false)
		{
			...
			if(!empty($_POST['send']))
			{
				...
				foreach($tags as $tag => $value)
				{
				    $update = array();
				    $update['value'] = $value;
				    $this->db->update('musca_i18n',$update,"section='$section' AND lang='$lang' AND tag='$tag'");
				}
				$this->smarty->assign('saved', true);
			}
		    ...
		    $res = $this->db->getAll("SELECT * FROM musca_i18n WHERE section='$section' AND lang in ('$lang_start', '$lang_end') ORDER BY id_i18n");
		    $return = array();
		    foreach($res as $k => $v) 
		    {
		...
		function del()
		{
			...
			$del = $this->db->getAssoc("SELECT tag, section FROM musca_i18n WHERE id_i18n IN (".rtrim($_POST['ids'],",").")");
