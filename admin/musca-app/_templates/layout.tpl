...
<head>
	...
	<title>{t section="meta_tags"}CMS{/t}</title>

	<link rel="shortcut icon" href="{$musca_url}/favicon.ico" type="image/x-icon">
	<link rel="icon" href="{$musca_url}/favicon.ico" type="image/x-icon">
	...
	<title>{$smarty.server.SERVER_NAME}</title>
</head>
<body class="{$list}">
	<div id="logout">
		...
		{/if}
		 <big>{$smarty.session[$smarty.const.CAPSULE]['auth']['name']}</big> &middot;
